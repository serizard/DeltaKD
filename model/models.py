...
def load_teacher_student_model(teacher_model_name, student_model_name, drop_path_rate=0.1, args=None):
    ...
    if distillation_type.lower() == 'vitkd':
        ...
        student_model.align = nn.ModuleList([
            nn.Linear(student_model.embed_dim, teacher_dims, bias=True)
            for _ in range(3)
        ])
    ...
    elif distillation_type.lower() == 'mgd':
        student_model.align = nn.Linear(student_model.embed_dim, teacher_model.embed_dim, bias=True)
        student_model.mask_token = nn.Parameter(torch.zeros(1, 1, teacher_model.embed_dim))
        student_model.generation = nn.Sequential(
            nn.Conv2d(teacher_model.embed_dim, teacher_model.embed_dim, kernel_size=3, padding=1),
            nn.ReLU(inplace=True), 
            nn.Conv2d(teacher_model.embed_dim, teacher_model.embed_dim, kernel_size=3, padding=1))

    elif distillation_type.lower() == 'curkd':
        student_dims = student_model.embed_dim
        teacher_dims = teacher_model.embed_dim
        student_model.curkd_align_early = nn.ModuleList([
            nn.Linear(student_dims, teacher_dims, bias=True) for _ in range(3)
        ])
        student_model.curkd_align_mid = nn.ModuleList([
            nn.Linear(student_dims, teacher_dims, bias=True) for _ in range(4)
        ])
        student_model.curkd_align_last = nn.Linear(student_dims, teacher_dims, bias=True)
        student_model.mask_token = nn.Parameter(torch.zeros(1, 1, teacher_dims))
        student_model.generation = nn.Sequential(
            nn.Conv2d(teacher_dims, teacher_dims, kernel_size=3, padding=1),
            nn.ReLU(inplace=True), 
    ...
    # 새로운 WassKD 분기 추가
    elif distillation_type.lower() == 'wasskd':
        student_dims = student_model.embed_dim
        teacher_dims = teacher_model.embed_dim
        # 학생 모델의 0,1,2번째 feature에 대해 학생의 feature를 teacher 차원으로 맞추기 위한 alignment module
        student_model.align_wasskd = nn.ModuleList([
            nn.Linear(student_dims, teacher_dims, bias=True) for _ in range(3)
        ])
